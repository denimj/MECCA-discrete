        ...
        for (j in 1:nchar) {
            ...
            for (k in 1:nsims) {
                for (i in 1:nbranches) {
                  if (as.numeric(phy$edge[i, 1]) == -1) 
                    s <- root.state
                  else {
                    parent <- which(phy$edge[, 2] == phy$edge[i, 
                      1])
                    s <- node.value[parent]
                  }
                ...
                result[, j, k] <- node.value[as.numeric(phy$edge[, 
                  2]) > 0]
            }
        }
        tt<-as.numeric(phy$edge[,2])
        oo<-tt[tt>0]
        rownames(result) <- phy$tip.label[oo]
    }
    return(result)
}
