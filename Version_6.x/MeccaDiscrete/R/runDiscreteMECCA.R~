runDiscreteMECCA <-
function(phy, tipStates, startState = startState, Ngens = 10000, printFreq = 100, qPrior=c(0.001, 1), scale = 1, divSampleFreq = 0,  summarizeSummaries = FALSE, outputName ="discreteMecca", seed=1) {  ## we need the priors in here!
...
## To run MECCA, the following arguments are required
	...
	#name.check(phy, tipStates) -> nc; #DCS: Same number of elements in phy and richness | There are no 'richness' argument. I changed it to 'tipStates'.
	#if(!nc == "OK") { stop("names in tree and data do not match") }
	#if(!sum(is.na(tipStates) == 0)) { stop("tipStates contains missing values") }
	phy$node.label <- NULL

	m <- match(phy$tip.label, rownames(tipStates))
	tipStates <- tipStates[m,]
	       
	richness <- rowSums(tipStates)
