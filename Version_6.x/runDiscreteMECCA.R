runDiscreteMECCA <-
function(phy, richness, startState = startState, Ngens = 10000, printFreq = 100, qPrior=c(0.001, 1), scale = 1, divSampleFreq = 0,  summarizeSummaries = FALSE, outputName ="discreteMecca", seed=1) {  ## we need the priors in here!
...
## To run MECCA, the following arguments are required
	...
	m <- match(phy$tip.label, rownames(richness))
	tipStates <- tipStates[m,]
	       
	rich <- rowSums(richness)
	...
	Lk <- logLP(phy, thetaB, thetaD) + logLT(phy, rich, thetaB, thetaD)
	...
	propLk <- logLP(phy, thetaBprop, thetaDprop) + logLT(phy, rich, thetaBprop, thetaDprop) # compute the joint taxonomic/phylogenetic likelihood of the tree/richness data based on propB and propD
	...
	simData <- simDiscreteIncompleteTree(phy, rich, q12= thetaQ12prop, q21= thetaQ21prop, rootState=rootState, r= thetaB-thetaD, eps=thetaD/thetaB, seed) ## DCS: This line was using 'mtree' before, this object does not exist.
	...
	sims <- simData$res[,1]/rowSums(simData$res)
 	 ...
 	 #proposalMat[ngen, ] <- c(thetaSprop, thetaMprop, dSim, sims);
   		...
   		if (dSim <= dcrit) {## if it's odd assess q's
				...
				Sim <- simDiscreteIncompleteTree(phy, rich, q12= thetaQ12, q21= thetaQ21, rootState=rootState, r= thetaB-thetaD, eps=thetaD/thetaB) ##DCS: Line was using 'mtree'. This object does not exist.
